...
public class ExcelBarStackedChart implements ExcelChart {
	...
	public void bulider(XSSFSheet sheet,XSSFChart chart, ChartOption options) {
		...
		xAxis.setCrossBetween(AxisCrossBetween.BETWEEN);// 两端留白
		// 标签样式
		...
		XDDFBarChartData bar = (XDDFBarChartData) chart.createData(ChartTypes.BAR, yAxis, xAxis);
		bar.setBarGrouping(BarGrouping.STACKED);// 叠加条形图
		...
		for (int i = 1; i < dimensions.length; i++) {
			...
			Series series = bar.addSeries(xAxisSource, yAxisSource);
