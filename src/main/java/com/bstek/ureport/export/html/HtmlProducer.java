...
public class HtmlProducer {
	...
	public String produce(Context context, List<Page> pages, int columnMargin, boolean breakPage) {
		...
		// 仅分页不分栏直接返回结果
		if(pageSize == 1) {
			return produce(context, pages.get(0), false);
		}
...
//		}
		...
		if (breakPage) {
			sb.append("<table border='0' class='page-break' style='margin:auto;border-collapse:collapse;width:"
					+ tableWidth + "pt" + bgStyle + "'>");
		} else {
			sb.append("<table border='0' class='page-break' style='margin:auto;border-collapse:collapse;width:"
					+ tableWidth + "pt" + bgStyle + "'>");
		}
	...
	private StringBuilder buildTable(Context context, List<Row> rows, List<Column> columns,
			Map<Row, Map<Column, Cell>> cellMap, boolean breakPage, boolean forPage) {
		StringBuilder sb = new StringBuilder();
...
//		}
		if (breakPage) {
			sb.append("<table class='page-break' border='0' style='word-wrap:break-word;word-break:break-all;white-space:normal;margin:auto;border-collapse:collapse;width:"
					+ tableWidth + "pt" + bgStyle + "'>");
		} else {
		...
		for (int i = 0; i < rowSize; i++) {
			...
			for (int j = 0; j < colSize; j++) {
				...
				if (obj instanceof Image) {
					...
					sb.append("<div style=\"width:" + width + "pt;height:" + height + "pt\">");
